...
export const metadata: Metadata = {
  ...
  description: 'فشرده‌سازی و تغییر اندازه تصاویر مخصوص بازار ایران و پیام‌رسان‌ها.',
}

export default function ImageToolsPage() {
  const category = toolCategories.find((c) => c.id === 'image')
  const items = toolsByCategory('image')

  return (
    <div className="space-y-6">
      <header className="space-y-2">
        <p className="text-sm font-semibold text-primary">{category?.titleFa}</p>
        <h1 className="text-2xl font-bold">بهینه‌سازی تصاویر برای وب و پیام‌رسان</h1>

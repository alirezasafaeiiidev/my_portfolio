import type { Metadata } from 'next'
import Link from 'next/link'
import { toolCategories, toolsByCategory } from '@/lib/tools-registry'

export const metadata: Metadata = {
  title: 'ابزارهای متن',
  description: 'پاکسازی و فرمت سریع متن‌های فارسی و انگلیسی.',
}

export default function TextToolsPage() {
  const category = toolCategories.find((c) => c.id === 'text')
  const items = toolsByCategory('text')

  return (
    <div className="space-y-6">
      <header className="space-y-2">
        <p className="text-sm font-semibold text-primary">{category?.titleFa}</p>
        <h1 className="text-2xl font-bold">متن مرتب و آماده ارسال</h1>
        <p className="text-muted-foreground">{category?.descriptionFa}</p>
      </header>

      {items.length === 0 ? (
        <p className="rounded-md border border-dashed border-border bg-background p-4 text-sm text-muted-foreground">
          به‌زودی ابزارهای متن اضافه می‌شود.
        </p>
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {items.map((tool) => (
            <Link
              key={tool.id}
              href={tool.slug}
              className="block rounded-lg border border-border bg-background p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
            >
              <h2 className="font-semibold">{tool.titleFa}</h2>
              <p className="text-sm text-muted-foreground mt-2">{tool.descriptionFa}</p>
            </Link>
          ))}
        </div>
      )}
    </div>
  )
}

...
if [[ -z "$TARGET_RELEASE" ]]; then
  ...
  if (( ${#releases[@]} < 2 )); then
    echo "[rollback] at least two releases are required" >&2
    exit 1
  fi
...
if [[ ! -d "$TARGET_DIR" || ! -f "$TARGET_DIR/ecosystem.config.cjs" ]]; then
  echo "[rollback] invalid target release: $TARGET_DIR" >&2
  exit 1
fi
